...
pub async fn execute(config: &Config, format: Option<crate::GenerateFormat>, version: Option<String>, output: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_dir = output.unwrap_or_else(|| "./gen".to_string());
    ...
    for schema in schemas {
        ...
        write_json(&output_path, &schema_info)?;
    }

    Ok(())
}

fn write_json(output_dir: &PathBuf, schema_info: &inspect::SchemaInfo) -> Result<(), Box<dyn std::error::Error>> {
