...
BEGIN
    -- Create test user
    INSERT INTO "auth"."user" DEFAULT VALUES RETURNING "id" INTO test_user_id;
    ...
    VALUES (test_comment_id, test_user_id, 'Test Comment') 
    RETURNING "id" INTO test_comment_id;

    -- Create test vote authors
    INSERT INTO "auth"."user" DEFAULT VALUES RETURNING "id" INTO test_author1;
    INSERT INTO "auth"."user" DEFAULT VALUES RETURNING "id" INTO test_author2;
    ...
    -- TEST: Add upvote
    PERFORM "comment_vote"(test_comment_id, test_author1, 1);
    ...
    PERFORM "comment_vote"(test_comment_id, test_author2, 1);
    SELECT "rank" INTO current_rank FROM "comment" WHERE "id" = test_comment_id;
    ...
    SELECT "rank" INTO current_rank FROM "comment" WHERE "id" = test_comment_id;
    IF current_rank != -1 THEN
        RETURN format('[d3e4f5a6] rank=%s expected=-1', current_rank);
    END IF;
    ...
    SELECT "rank" INTO current_rank FROM "comment" WHERE "id" = test_comment_id;
    IF current_rank != 1 THEN
        RETURN format('[f5a6b7c8] rank=%s expected=1', current_rank);
    END IF;
    SELECT "vote" INTO vote_value FROM "comment_action" WHERE "id" = test_comment_id AND "author" = test_author1;
    IF vote_value != 'up' THEN
    ...
    -- TEST: Remove vote
    PERFORM "comment_vote"(test_comment_id, test_author1, 0);
    SELECT "rank" INTO current_rank FROM "comment" WHERE "id" = test_comment_id;
    IF current_rank != 0 THEN
        RETURN format('[b7c8d9e0] rank=%s expected=0', current_rank);
    END IF;
    SELECT EXISTS(SELECT 1 FROM "comment_action" WHERE "id" = test_comment_id AND "author" = test_author_id) INTO vote_exists;
    IF vote_exists THEN
    ...
    SELECT "rank" INTO current_rank FROM "comment" WHERE "id" = test_comment_id;
    IF current_rank != 0 THEN
        RETURN format('[d9e0f1a2] rank=%s expected=0', current_rank);
    END IF;
    ...
    DELETE FROM "story" WHERE "id" = test_comment_id;
